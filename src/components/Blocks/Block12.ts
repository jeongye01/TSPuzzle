...
const blockShape=[[1,1,0],[0,0,0],[0,0,0]];
export function Block12 (diffSetter:(x:number,y:number)=>void,blockSetter:(block:HTMLDivElement,fillFunc:(x:number,y:number)=>void,overFunc:(x:number,y:number)=>void)=>void){
...
export const fillBlock =(x:number,y:number)=>{
 ...
 // console.log(e.target.dataset);
  ...
  if(  boardState[x][y] || boardState[x+1][y])return;
